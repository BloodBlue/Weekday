  ...
  data () {
    return {
      ...
      statuslist: [
        ...
        {index: 7, title: '调查问卷', date: '', percentage: 0, color: 'rgba(142, 113, 199, 0.7)'}
      ]
    }
  },
  watch: {
  ...
  methods: {
    ...
    getStatus () {
      ...
      for (var key in this.date) {
        ...
        this.statuslist[index].title = this.get_title(this.date[key])
        index = index + 1
      }
      index = 0
      for (var key1 in this.day) {
        for (var key2 in this.statuslist) {
          if (this.statuslist[key2].title === this.get_title(key1)) {
            this.statuslist[key2].percentage = 100
          }
        }
      }
    }
